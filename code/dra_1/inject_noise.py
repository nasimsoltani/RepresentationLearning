...
def inject_isotropic_noise(z, noise_level, clip_value=None):
    """
    Injects isotropic Gaussian noise with a specified total variance and applies
    clipping by scaling to min(original norm, clip_value) when noise is added.
    If no noise is added, optionally clip the original tensor.
    """
    if noise_level > 0:
        ...
        per_dim_stddev = torch.sqrt(
            torch.tensor(per_dim_variance, device=z.device, dtype=z.dtype)
        )

        epsilon = torch.randn_like(z_normalized) * per_dim_stddev

        # Add noise to the normalized vector
        z_noisy_normalized = z_normalized + epsilon

        # Scale back to min(original norm, clip_value) when clip_value is provided
        target_norm = z_norm if clip_value is None else torch.clamp(z_norm, max=clip_value)
        z_noisy = z_noisy_normalized * target_norm
        return z_noisy

    # No noise case: optionally clip by scaling down to clip_value
    if clip_value is not None:
        z_norm = torch.linalg.norm(z, dim=1, keepdim=True)
        clip_factor = torch.clamp(clip_value / (z_norm + 1e-7), max=1.0)
        return z * clip_factor
    return z


def inject_nonisotropic_noise(z, noise_level, L, V, lambda_factor=1e-5, clip_value=None):
    """
    Injects anisotropic Gaussian noise based on FIM eigendecomposition and applies
    clipping by scaling to min(original norm, clip_value) when noise is added.
    If no noise is added (or L/V missing), optionally clip the original tensor.
    """
    ...
    # No noise (or missing L/V) case: optionally clip by scaling down to clip_value
    if clip_value is not None:
        z_norm = torch.linalg.norm(z, dim=1, keepdim=True)
        clip_factor = torch.clamp(clip_value / (z_norm + 1e-7), max=1.0)
        return z * clip_factor
