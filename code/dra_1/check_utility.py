...
from rep_lr.models import RFClassificationHead, ChannelEstimationHead, CFOEstimationHead
...
def main():
    ...
    # ---- Automatically find model, activations, and partition file ----
    experiment_path = cli_args.experiment_path

    # Find model checkpoint (.pt file)
    model_path = None
    for f in sorted(os.listdir(experiment_path), reverse=True): # Find best/latest
        if f.endswith(".pt"):
    ...
    if is_mtl:
        print("Reconstructing MTL heads.")
        ...
        print("Reconstructing single-task head.")
        task = train_args.task
        if task == 'rf_fingerprinting':
            head = RFClassificationHead(input_dim=2*train_args.d2, num_classes=num_classes, hidden_dim=train_args.head_hidden_dim, dropout=train_args.dropout)
        elif task == 'channel_estimation':
            head = ChannelEstimationHead(input_dim=2*train_args.d2, hidden_dim=train_args.head_hidden_dim, output_length=52, dropout=train_args.dropout)
        elif task == 'cfo_estimation':
            head = CFOEstimationHead(input_dim=2*train_args.d2, hidden_dim=train_args.head_hidden_dim, dropout=train_args.dropout)
        else:
        ...
        if 'head_state_dict' in checkpoint:
            ...
            raise KeyError("Could not find head weights in checkpoint.")
        heads[task] = head
