...
def main(args):
    ...
    wandb.init(
        project="data-reconstruction-attack-run-1-v131",
        config=wandb_config,
        name=wandb_name
    )
    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{args.epochs} [Train]"):
            ...
            if args.noise_type == 'isotropic':
                ...
                activations = inject_nonisotropic_noise(activations, args.noise_level, L, V, args.lambda_factor)
...
# ## MODIFICATION START: Added Total Variation Loss function
# def total_variation_loss(recons, weight):
#     """
#     Calculates the Total Variation (TV) loss for a batch of 1D signals.
#     The input shape is expected to be (B, C, L).
#     """
#     if weight == 0:
#         return 0.0
#     # Calculate the differences between adjacent time steps along the length dimension
#     tv_loss = torch.sum(torch.abs(recons[:, :, :-1] - recons[:, :, 1:]))
...
#     if args.noise_type == 'nonisotropic':
#         wandb_name += f"_lambda_{args.lambda_factor}"
#     ## MODIFICATION START: Added TV weight to wandb name for clarity
#     if args.tv_weight > 0:
#         wandb_name += f"_tv_{args.tv_weight}"
#     ## MODIFICATION END
#     wandb.init(
#         project="data-reconstruction-attack-run-1-v11",
#         config=wandb_config,
#         name=wandb_name
#     )
...
#         else:
#             print("Model is single-task. Calculating FIM for the specific task...")
#             model_task_name = train_args.task
#             task_map = {'rf': 'rf_fingerprinting', 'cfo': 'cfo_estimation', 'channel': 'channel_estimation'}
#             attack_task_name = task_map[args.task]
...
#         decoder.train()
#         total_train_loss = 0
#         ## MODIFICATION START: Added accumulator for TV loss logging
#         total_train_tv_loss = 0
#         ## MODIFICATION END
#         for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{args.epochs} [Train]"):
#             rf_x, _, cfo_x, _, channel_x, _, activations, _ = batch
#             activations = activations.squeeze(1).to(device)
...
#             ## MODIFICATION START: Calculate combined loss with TV regularization
#             loss_mse = criterion(recons, true_data)
#             loss_tv = total_variation_loss(recons, args.tv_weight)
...
#             total_train_loss += loss.item()
#             ## MODIFICATION START: Accumulate TV loss
#             if isinstance(loss_tv, torch.Tensor):
#                 total_train_tv_loss += loss_tv.item()
#             ## MODIFICATION END
        
#         avg_train_loss = total_train_loss / len(train_loader)
#         ## MODIFICATION START: Calculate average TV loss
#         avg_train_tv_loss = total_train_tv_loss / len(train_loader)
#         ## MODIFICATION END
...
#                 ## MODIFICATION START: Calculate combined loss with TV regularization for validation
#                 loss_mse = criterion(recons, true_data)
#                 loss_tv = total_variation_loss(recons, args.tv_weight)
#                 loss = loss_mse + loss_tv
#                 ## MODIFICATION END
...
#         current_lr = optimizer.param_groups[0]['lr']
#         ## MODIFICATION START: Log TV loss to wandb
#         wandb.log({
#             "epoch": epoch, 
#             "train_loss": avg_train_loss, 
#             "val_loss": avg_val_loss, 
#             "learning_rate": current_lr,
#             "train_tv_loss": avg_train_tv_loss
#         })
#         ## MODIFICATION END
...
#     ## MODIFICATION START: Added command-line argument for TV regularization weight
#     parser.add_argument('--tv_weight', type=float, default=1e-5, help="Weight for Total Variation regularization. Encourages smoother reconstructions. A small value like 1e-6 is a good start.")
#     ## MODIFICATION END
