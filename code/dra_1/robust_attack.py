    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{args.epochs} [Train]"):
            ...
            if args.noise_type == 'isotropic':
                activations = inject_isotropic_noise(activations, args.noise_level, clip_value=args.clip_value)
            elif args.noise_type == 'nonisotropic':
                activations = inject_nonisotropic_noise(activations, args.noise_level, L, V, args.lambda_factor, clip_value=args.clip_value)
            elif args.noise_type == 'none' and args.clip_value is not None:
                # Still apply clipping even with no noise
                activations = inject_isotropic_noise(activations, 0.0, clip_value=args.clip_value)
